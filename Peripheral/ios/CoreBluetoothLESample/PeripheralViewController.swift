...
class PeripheralViewController: UIViewController {
    ...
    public struct Configuation: Codable {
        let type: Int
        let address: String
        let message: String
    }

    public let configurationToSend: Configuation = Configuation(type: 5, address: "10.12.15.12", message: "Test Message")

    var transferCharacteristicRx: CBMutableCharacteristic?
    var transferCharacteristicTx: CBMutableCharacteristic?
    ...
    override func viewDidLoad() {
        ...
        let data = try? JSONEncoder().encode(configurationToSend)
    ...
    private func sendData() {
		...
		guard let transferCharacteristic = transferCharacteristicTx else {
			return
		}
    ...
    private func setupPeripheral() {
        ...
        let transferRxCharacteristic = CBMutableCharacteristic(type: TransferService.characteristicRxUUID,
                                                             properties: [.writeWithoutResponse],
                                                         value: nil,
                                                         permissions: [.writeable])

        let transferTxCharacteristic = CBMutableCharacteristic(type: TransferService.characteristicTxUUID,
        ...
        transferService.characteristics = [transferTxCharacteristic, transferRxCharacteristic]
        
        // And add it to the peripheral manager.
        peripheralManager.add(transferService)
        
        // Save the characteristic for later.
        self.transferCharacteristicTx = transferTxCharacteristic
        self.transferCharacteristicRx = transferRxCharacteristic
        ...
        for aRequest in requests {
            ...
            if stringFromData == "EOM" {
                ...
                DispatchQueue.main.async() {
                    let command = String(data: self.data, encoding: .utf8)

                    if command?.uppercased() == "echo".uppercased() {
                        self.dataToSend = command!.data(using: .utf8)!
                    } else if command?.uppercased() == "config".uppercased() {
                        self.dataToSend = self.textView.text.data(using: .utf8)!
                    } else {
                        self.dataToSend = "Unknown".data(using: .utf8)!
                    }
